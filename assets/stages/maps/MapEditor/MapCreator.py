    ...
    def get_event(type, x, y, affects):
        ...
        item = {
            ...
            "hitbox": {
                ...
                "height": (rect[f][2] - rect[f][0] + 1) * basic_size,
            },
            "affect": affects,
        }
        return item
        ...
        while not q.empty():
            ...
            for k in range(4):
                ...
                if C[nx][ny] == C[x][y]:
                    q.put((nx, ny, k))
                # print(x, y, nx, ny)
                events[get_event_name(x, y)] = get_event(
                    C[x][y], x, y, [get_event_name(nx, ny)]
                )
    ...
    for i in range(18):
        for j in range(32):
            ...
            if C[x][y] == 1 or C[x][y] == 1.5:
                bfs(x + 1, y, 2)
                events[this_name] = get_event(
                    int(C[x][y]), x, y, [get_event_name(x + 1, y)]
                )
                if C[x][y] == 1:
                    events[this_name]["hitbox"]["x"] -= basic_size // 2
                events[this_name]["hitbox"]["width"] += basic_size
                events[this_name]["hitbox"]["height"] //= 2
                events[this_name]["hitbox"]["y"] += basic_size // 2
