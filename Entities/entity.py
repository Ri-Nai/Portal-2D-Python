    ...
    def updateJump(self, isSpaceHeld: bool):
        if self.isJumping:
            if not self.isFalling and isSpaceHeld and self.chargeTime < self.maxJump:
                ...
                self.chargeTime += 1
                self.jumpVelocity = min(
                    self.baseJump
                    + (self.chargeTime / self.maxJump) * (self.maxJump - self.baseJump),
                    self.maxJump,
                )
            else:
                self.isFalling = True
...
class Entity:
    ...
    def moveOutPortalPosition(self, portal: Portal):
        ...
        if Hitbox(new_position, self.hitbox.get_size()).checkHits(
            Game.get_instance().map_manager.super_edges
        ):
            return None
    ...
    def rigidMove(self):
        ...
        def moveDirection(delta: Vector, value):
            ...
            collided = self.hitbox.checkHits(
                Game.get_instance().map_manager.super_edges,
                lambda: self.hitbox.move_ip(-delta[0], -delta[1]),
            )
            if collided:
                flag |= 1 << value
            return not collided
    ...
    def updateX(self, move):
        ...
        if not self.is_flying and abs(self.velocity.x) <= self.MaxSpeed:
            if move == 0:
                ...
                nextVelocityX = move * min(
                    sqrt(self.velocity.x * self.velocity.x + 10), self.MaxSpeed
                )
        else:
            if move == 0:
                if onGround:
                    nextVelocityX = decelerate(
                        self.velocity.x, (0.16) * (1 + self.is_player)
                    )
                else:
                    nextVelocityX = decelerate(
                        self.velocity.x, (0.01) * (1 + self.is_player)
                    )
    ...
    def check_out_of_map(self):
        ...
        if not self.hitbox.colliderect(
            Hitbox(
                0,
                0,
                Game.get_instance().screen.get_width(),
                Game.get_instance().screen.get_height(),
            )
        ):
